...
const MessagePage = () => {
  ...
  // Récupérer tous les utilisateurs et filtrer ceux qui sont dans matches
  useEffect(() => {
    const fetchMatches = async () => {
      ...
      // Matches = uniquement ceux présents dans currentUser.matches
  ...
  //       // Création automatique si la conversation n'existe pas
  ...
  const handleSendMessage = async () => {
    ...
    const msgStr = `${timestamp} true read_true ${newMessage}`;
  ...
  const parseMessages = (items) => {
    ...
    return items.map(item => {
      // item = "timestamp true read_true texte"
      const firstSpace = item.indexOf(" ");
      const secondSpace = item.indexOf(" ", firstSpace + 1);
      const thirdSpace = item.indexOf(" ", secondSpace + 1);

      const timestamp = Number(item.substring(0, firstSpace));
      const fromMe = item.substring(firstSpace + 1, secondSpace) === "true";
      const read = item.substring(secondSpace + 1, thirdSpace) === "read_true";
      const text = item.substring(thirdSpace + 1);
              ...
              <>
                ...
                <div className="space-y-3">
                  {messages.map((m, idx) => (
                    <div
                      key={idx}
                      className={`p-3 rounded-2xl max-w-xs shadow-md break-words ${m.fromMe ? "bg-pink-600 text-white ml-auto" : "bg-white text-gray-800"
                        }`}
                    >
